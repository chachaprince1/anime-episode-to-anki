(function () {
  ...
  function render() {
    ...
    if (state.jpdb === null) {
      ...
      setConnection("jpdb", "ok", "Connected", "Your API key is valid.");
    } else {
      setConnection("jpdb", "bad", "Action needed", "Open jpdb settings once and securely connect the account signed in to this Chrome profile.");
    }

    if (state.yomitan === null) {
      setConnection("yomitan", "checking", "Checking", "Checking the local Yomitan bridge…");
    } else if (!state.yomitan.ok) {
      setConnection("yomitan", "bad", "Action needed", "Download the helper for your system, run it once, then turn on Advanced and Yomitan API under General.");
    ...
    if (exportReady && ankiReady) {
      ...
      byId("ready-title").textContent = "You’re ready to add cards";
      byId("ready-detail").textContent = "Open a jpdb anime page and click Add to Anki.";
    } else if (exportReady) {
  ...
  function bindEvents() {
    byId("show-api-key").addEventListener("change", (event) => {
      byId("jpdb-api-key").type = event.target.checked ? "text" : "password";
    });
