...
$normalizedSource = ([IO.File]::ReadAllText($source)).Replace("`r`n", "`n")
$normalizedBytes = [Text.UTF8Encoding]::new($false).GetBytes($normalizedSource)
$actual = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($normalizedBytes)).ToLowerInvariant()
if ($actual -ne $expected) { throw 'Pinned Yomitan helper checksum did not match.' }
